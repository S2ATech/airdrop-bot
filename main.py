...
def menu(id):
    ...
    bot.send_message(id,"*🏡 Menu ⬇️*", parse_mode="Markdown", reply_markup=keyboard)
   ...
   try:
    ...
    if msg == '/start':
        ...
        markup.add(telebot.types.InlineKeyboardButton(
           text='🤼‍♂️ Vérifier', callback_data='check'))
    ...
    if call.data == 'check':
        if ch == True:
            ...
            bot.answer_callback_query(
                callback_query_id=call.id, text='✅ Vous avez rejoins avec succès')
            ...
            if user not in data['refer']:
                ...
                if int(data['referby'][user]) != user_id:
                    ...
                    data['referred'][ref] += 1
                    bot.send_message(
                        ref_id, f"*🏧 Nouveau referral en Level 1, vous avez : +{Per_Refer} {TOKEN}*", parse_mode="Markdown")
                    json.dump(data, open('users.json', 'w'))
            ...
            bot.answer_callback_query(
                callback_query_id=call.id, text='❌  ous devez rejoindre les canaux')
            bot.delete_message(call.message.chat.id, call.message.message_id)
            markup = telebot.types.InlineKeyboardMarkup()
            markup.add(telebot.types.InlineKeyboardButton(
                text='🤼‍♂️ Vérifier', callback_data='check'))
...
def send_text(message):
   try:
    ...
    if message.text == "⚙️ Mettre un numéro":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Entrez votre numéro de téléphone suivie du code pays._",
    ...
    if message.text == "🎁 Bonus":
        ...
        if (user_id not in bonus.keys()) or (cur_time - bonus[user_id] > 60*60*24):
            ...
            bot.send_message(
                user_id, f"Félicitation vous avez {Daily_bonus} {TOKEN}")
            bonus[user_id] = cur_time
            json.dump(data, open('users.json', 'w'))
        else:
            bot.send_message(
                message.chat.id, "❌*Vous pour recevoir un autre bonus dans 24h*",parse_mode="markdown")
        return
    ...
    if message.text == "📊Statistiques":
        ...
        user = str(user_id)
        data = json.load(open('users.json', 'r'))
        msg = "*📊 Total parrainées : {} Users\n\n🥊 Parrainez pour gagner plus*"
        ref_count = data['referred'][user]
        msg = msg.format(ref_count)
    ...
    if message.text == "💸 Retrait":
        ...
        if wall == "none":
            bot.send_message(user_id, "_❌ Vous n'avez pas un numéro_",
                             parse_mode="Markdown")
            return
        if bal >= Mini_Withdraw:
            ...
            bot.send_message(
                user_id, f"_❌ Votre solde dois être supérieur ou égal à {Mini_Withdraw} {TOKEN} pour retirer_", parse_mode="Markdown")
            return
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
...
def trx_address(message):
   try:
    ...
    if len(message.text) == 34:
        ...
        bot.send_message(message.chat.id, "*💹 Votre numéro de téléphone est mise à jour " +
                         data['wallet'][user]+"*", parse_mode="Markdown")
        json.dump(data, open('users.json', 'w'))
        return menu(message.chat.id)
    else:
        bot.send_message(
            message.chat.id, "*⚠️ Votre numéro est invalide*", parse_mode="Markdown")
        return menu(message.chat.id)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
...
def amo_with(message):
   try:
    ...
    if msg.isdigit() == False:
        bot.send_message(
            user_id, "_📛 voleur invalide Entrez les chiffres uniquement. Ressayez", parse_mode="Markdown")
        return
    if int(message.text) < Mini_Withdraw:
        bot.send_message(
            user_id, f"_❌  Le minimum de retrait est {Mini_Withdraw} {TOKEN}_", parse_mode="Markdown")
        return
    if int(message.text) > bal:
        bot.send_message(
            user_id, "_❌  Le  font est insuffisant veuillez invité des amis pour gagner plus_", parse_mode="Markdown")
        return
    amo = int(amo)
    ...
    bot_name = bot.get_me().username
    json.dump(data, open('users.json', 'w'))
    bot.send_message(user_id, "✅* Votre demande retrait est automatiquement envoyé dans \n\n💹 Notre canal:- "+PAYMENT_CHANNEL +"*", parse_mode="Markdown")

    markupp = telebot.types.InlineKeyboardMarkup()
    markupp.add(telebot.types.InlineKeyboardButton(text='🍀 Lien du bot', url=f'https://telegram.me/{bot_name}?start={OWNER_ID}'))
    ...
    send = bot.send_message(PAYMENT_CHANNEL,  "✅* Nouveau retrait\n\n⭐ Amount - " + str(amo) +
            ...
            "\n⏩ svp veuillez attendre la confirmation*", parse_mode="Markdown", disable_web_page_preview=True, reply_markup=markupp)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
        return
