...
BOT_TOKEN = "5111266579:AAH1_381DxTME9zcIEkbUvYCdrW9-29RK0o"
...
def menu(id):
    ...
    bot.send_message(id, "*🏡 Menu ⬇️*", parse_mode="Markdown",
   ...
   try:
    ...
    if msg == '/start':
        ...
        markup.add(telebot.types.InlineKeyboardButton(
           text='🤼‍♂️ Vérifié', callback_data='check'))
   ...
   try:
    ...
    if call.data == 'check':
        if ch == True:
            ...
            bot.answer_callback_query(
                callback_query_id=call.id, text='❌  Vous devez rejoindre les canaux')
            bot.delete_message(call.message.chat.id, call.message.message_id)
            markup = telebot.types.InlineKeyboardMarkup()
            markup.add(telebot.types.InlineKeyboardButton(
                text='🤼‍♂️ Vérifié', callback_data='check'))
   ...
   try:
    ...
    if message.text == "💸 Retrait":
        ...
        if bal >= Mini_Withdraw:
            bot.send_message(user_id, "_Entrer le solde_",
                             parse_mode="Markdown")
...
def trx_address(message):
   try:
    ...
    if len(message.text) >= 11:
        ...
        bot.send_message(message.chat.id, "*💹 Votre numéro est mis avec succès " +
                         data['wallet'][user]+"*", parse_mode="Markdown")
        json.dump(data, open('users.json', 'w'))
        return menu(message.chat.id)
    else:
        bot.send_message(
            message.chat.id, "*⚠️ Numéro invalide!*", parse_mode="Markdown")
        return menu(message.chat.id)
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
